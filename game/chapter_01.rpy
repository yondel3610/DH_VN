...
#
# ┌─────────────────────────────────────────────────────────────────────────┐
# │  GUIDE: HOW BRANCHING STATEMENTS WORK / HOW ENDINGS DIFFER             │
# ├─────────────────────────────────────────────────────────────────────────┤
# │                                                                         │
# │  VARIABLES                                                              │
# │  ─────────                                                              │
# │  Variables are the memory of your game. They track what the player     │
# │  has done and determine what content they see later.                    │
# │                                                                         │
# │  'default variable_name = value'                                        │
# │      → Declares a variable with a starting value at game launch.      │
# │      → Integers (0, 1, 2...) for counters and affection scores.       │
# │      → Booleans (True/False) for one-time flags.                       │
# │      → Strings ("") for tracking choice labels.                        │
# │                                                                         │
# │  '$ variable_name = value'   (inside a label)                          │
# │      → Sets a variable during gameplay.                                │
# │      → '$ niko_affection += 1'  increments by 1.                      │
# │      → '$ city_visited_zhong = True'  sets a flag.                    │
# │                                                                         │
# │  AFFECTION TRACKERS (used in this chapter)                             │
# │  ──────────────────                                                     │
# │  niko_affection  — how much Niko trusts/likes Dorian                  │
# │  feng_affection  — how much Feng respects Dorian                       │
# │                                                                         │
# │  These accumulate across ALL chapters. By Chapter 6 and 10, the       │
# │  total score determines which romance ending you unlock, and whether   │
# │  characters appear to rescue you.                                       │
# │                                                                         │
# │  CONDITIONAL BLOCKS — if / elif / else                                 │
# │  ────────────────────────────────────                                  │
# │  'if condition:'     → runs this block if the condition is true        │
# │  'elif condition:'   → runs this block if the first was false          │
# │                         and this condition is true                     │
# │  'else:'             → runs this block if ALL above were false         │
# │                                                                         │
# │  Example — dialogue that changes based on Feng's affection score:     │
# │                                                                         │
# │      if feng_affection >= 2:                                           │
# │          feng "You've earned my respect, Dorian."                      │
# │      elif feng_affection == 1:                                         │
# │          feng "You're getting there, I suppose."                       │
# │      else:                                                             │
# │          feng "I'm watching you, paladin."                             │
# │                                                                         │
# │  JUMP vs CALL                                                           │
# │  ────────────                                                           │
# │  'jump label_name'   → moves to that label PERMANENTLY.               │
# │                         Never returns. Used for: chapter transitions,  │
# │                         GAME OVER screens, major route forks.          │
# │                                                                         │
# │  'call label_name'   → moves to that label TEMPORARILY.               │
# │                         Returns to the next line after 'call'          │
# │                         when the called label hits 'return'.           │
# │                         Used for: sub-scenes, repeatable menus.        │
# │                                                                         │
# │  HOW ENDINGS WORK IN THIS GAME                                         │
# │  ──────────────────────────────                                        │
# │  This chapter has three types of branching outcomes:                   │
# │                                                                         │
# │  1. SOFT BRANCHES (flavour only)                                       │
# │     D1 city exploration, D4 Elara chat topics.                         │
# │     Player sees different dialogue/scenes, but all paths converge       │
# │     to the same next scene. No variable change.                        │
# │                                                                         │
# │  2. TRACKER BRANCHES (accumulate across chapters)                      │
# │     D2 (+niko_affection), D3 (+feng_affection),                        │
# │     D7 (+yuki_tracker).                                                 │
# │     The score is checked later — in Chapter 6, 9, and 10 — to        │
# │     determine who saves you, what ending you get, and which            │
# │     romance route is active.                                           │
# │                                                                         │
# │  3. HARD GATES (GAME OVER)                                             │
# │     D8 (Taotie dodge), D9 (Taotie mouth), D10 (Taotie lava).         │
# │     Wrong answer = 'jump game_over'. No recovery.                     │
# │     Player must reload and replay from the last save point.           │
# │                                                                         │
# │  TRACKER SUMMARY FOR THIS CHAPTER:                                     │
# │  niko_affection  : +1 if D2 = Intervene                               │
# │  feng_affection  : +1 if D3 = Tell old man to shut up                 │
# │  yuki_tracker    : +1 if D7 = Stumble and fall (unlocks Yuki content) │
# └─────────────────────────────────────────────────────────────────────────┘
